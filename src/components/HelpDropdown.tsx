...
const HelpDropdown = () => {
  ...
  const helpPath = !currentService ? '' : `/${lang}/${currentService}/Welcome.html`;
  ...
  return (
    <DropdownMenu.Root>
      <DropdownMenu.Trigger asChild>
        <button className={styles['help-button']} type="button" aria-label="Help options">
          <img src={QuestionLogo} alt="Help icon" />
        </button>
      </DropdownMenu.Trigger>
      <DropdownMenu.Portal>
        <DropdownMenu.Content className={styles.help__content} sideOffset={5}>
          <DropdownMenu.Item className={styles['help-item']} onSelect={goToHelp}>
            Help & Asistance
          </DropdownMenu.Item>
